...
from .load_ner import load_data, load_data_with_raw, load_data_100_sample_with_raw
...
def judge_qualified(key_transformed, col):
    if col > 10 or col < 0:
        ...
        exit(0)
    qualified = True
    qualified = qualified and (len(np.array([x for x in key_transformed if x != 0 and x != -1])) >= 4)
    return qualified
...
def rank_cl_pl_pairs():
    ...
    while batch_size * batch_index < train_size:
        ...
        for j in range(len(input)):
            ...
            if not judge_qualified(feature, 2):
                continue
                ...
                for i in range(10):
                    if target[j][i] != -1:
                        ...
                        if (target[j][i], pred[i]) in raw_check:
                            with open('raw_to_deal_2_no_other/{}_{}.json'.format(new_key, cl_pl_count[
                                new_key]), 'w') as wfp:
                                json.dump(raw[j], wfp)

                            with open('raw_to_deal_2_no_other/{}_{}.txt'.format(new_key,
