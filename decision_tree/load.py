...
def load_data_100_sample(batch_size, batch_index=0):
    ...
    nst_inputs = numpy.array(
        [list(
            map(to_int, numpy.genfromtxt(os.path.join(training_data_100_sample_dir, batch_file_nst), delimiter=',')[0]))
         for
         batch_file_nst in batch_files_nst])
    date_inputs = numpy.array(
        [list(map(to_int, numpy.genfromtxt(os.path.join(training_data_100_sample_dir, batch_file_date), delimiter=',')))
         for
         batch_file_date in batch_files_date])
    targets = numpy.array(
...
if __name__ == '__main__':
    print(load_data_100_sample(10, 0))
