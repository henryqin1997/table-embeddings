...
training_data_12k_dir = 'data/domain_schema_files'
training_files_12k_json = 'data/domain_schema_files_dict.json'
training_files_12k = list(json.load(open(training_files_12k_json)).values())
...
def load_data_12k(batch_size, batch_index=0):
    ...
    # start from batch_index batch
    # return two arrays: input, target
    batch_files = training_files_12k[batch_size * batch_index:batch_size * (batch_index + 1)]
    batch_files_ner = list(map(lambda batch_file: batch_file.rstrip('.json') + '_ner.csv', batch_files))
    batch_files_wordlist = list(map(lambda batch_file: batch_file.rstrip('.json') + '_wordlist.csv', batch_files))
    inputs = numpy.array(
    ...
    # Use One Hot Encoding
    for i in range(len(inputs)):
        table = Table(json.load(open(os.path.join(training_data_12k_dir, batch_files[i]))))
        column_num = len(table.get_header())
        input = inputs[i]
        target = targets[i]
        assert len(input) == len(tag_to_index)
...
def load_data_12k_with_raw(batch_size, batch_index=0):
    ...
    batch_files_wordlist = list(map(lambda batch_file: batch_file.rstrip('.json') + '_wordlist.csv', batch_files))
    raws = numpy.array(
        [json.load(open(os.path.join(training_data_12k_dir, batch_file), encoding='utf-8')) for batch_file in
         batch_files])
    inputs = numpy.array(
        [numpy.genfromtxt(os.path.join(training_data_12k_dir, batch_file_ner), delimiter=',') for batch_file_ner in
         batch_files_ner])
