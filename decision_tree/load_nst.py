...
def load_nst_maxo(batch_size, batch_index):
    ...
    for i in range(len(targets)):
        ...
        targets_transformed.append(
            numpy.array([index_of(list(map(lambda num: int(round(num)), row)), 1) if idx < column_num else -1 for
                         idx, row in enumerate(target.transpose())]).transpose())
    return numpy.array([[x[1] * 2 + x[2] * 1 for x in zip(inputs[0], inputs[1], inputs[2])] for inputs in
                        zip(inputs_major, inputs_max, inputs_overall)]), numpy.array(targets_transformed)
...
def load_nst_mm(batch_size, batch_index):
    ...
    for i in range(len(targets)):
        ...
        targets_transformed.append(
            numpy.array([index_of(list(map(lambda num: int(round(num)), row)), 1) if idx < column_num else -1 for
                         idx, row in enumerate(target.transpose())]).transpose())
    return numpy.array([[x[0] * 4 + x[1] * 2 for x in zip(inputs[0], inputs[1], inputs[2])] for inputs in
                        zip(inputs_major, inputs_max, inputs_overall)]), numpy.array(targets_transformed)
...
def load_nst_mmo(batch_size, batch_index):
    ...
    for i in range(len(targets)):
        ...
        targets_transformed.append(
            numpy.array([index_of(list(map(lambda num: int(round(num)), row)), 1) if idx < column_num else -1 for
                         idx, row in enumerate(target.transpose())]).transpose())
    return numpy.array([[x[0] * 4 + x[1] * 2 + x[2] * 1 for x in zip(inputs[0], inputs[1], inputs[2])] for inputs in
                        zip(inputs_major, inputs_max, inputs_overall)]), numpy.array(targets_transformed)
